...
开发者可以通过[expandSafeArea](../reference/arkui-cj/cj-universal-attribute-expandsafearea.md#func-expandsafeareaarraysafeareatype-arraysafeareaedge)来开启沉浸式效果。
...
> **说明：**
>
> `safe-area-inset-*`由四个环境变量组成，分别定义了可视窗口边缘内矩形的`top`、`right`、`bottom`和`left`，确保内容可以安全地放置，避免被非矩形显示区域切断。在矩形视口中，这些值等于零。而对于非矩形显示器（例如圆形表盘、移动设备屏幕等），所有内容都将在用户代理设定的四个值所形成的矩形区域内可见。
