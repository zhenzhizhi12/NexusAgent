...
GATT处于未连接的状态下执行操作，例如调用[getServices](./cj-apis-bluetooth-ble.md#func-getservicesasynccallbackarraygattservice)和[readCharacteristicValue](./cj-apis-bluetooth-ble.md#func-readcharacteristicvalueblecharacteristic-asynccallbackblecharacteristic)。
...
读写特征值或描述符过于频繁，导致底层数据传输拥塞，例如入参[GattWriteType](./cj-apis-bluetooth-ble.md#enum-gattwritetype)为WriteNoResponse的[writeCharacteristicValue](./cj-apis-bluetooth-ble.md#func-writecharacteristicvalueblecharacteristic-gattwritetype-asynccallbackunit)接口调用过于频繁可能导致拥塞。

**处理步骤**

降低读写操作频率，如果是WriteNoResponse类型的写数据操作，建议调用间隔控制在50ms以上。
...
先调用[addService](./cj-apis-bluetooth-ble.md#func-addservicegattservice)添加该服务。
...
传入的广播标识符应为[startAdvertising](./cj-apis-bluetooth-ble.md#func-startadvertisingadvertisingparams)接口返回的值，无效的广播标识符默认为0xFF。
